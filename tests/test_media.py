...
def test_media_upload_uses_multipart_201_location_get_image_and_persists_resource(app):
    with app.app_context():
        ...
        assert method == "POST"
        assert args[0] == "https://apim.sandbox.ebay.com/commerce/media/v1_beta/image/create_image_from_file"
        ...
        assert method == "GET"
        assert args[0] == "https://apim.sandbox.ebay.com/commerce/media/v1_beta/image/fake-image-id"
        assert kwargs["headers"]["Authorization"] == "Bearer shared-token"
...
def test_media_service_uses_current_commerce_media_gateway_in_production(app):
    ...
    assert service.endpoint == "https://apim.ebay.com/commerce/media/v1_beta/image/create_image_from_file"
    assert service.image_endpoint("image id") == "https://apim.ebay.com/commerce/media/v1_beta/image/image%20id"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("sandbox", "https://api.sandbox.ebay.com/", "https://apim.sandbox.ebay.com"),
        ("production", "https://mock.example.test/api", "https://mock.example.test/api"),
    ],
)
def test_media_service_maps_canonical_api_base_to_media_gateway_but_preserves_custom_base(
    app, environment, configured, expected
):
    config = dict(app.config)
    config["EBAY_ENVIRONMENT"] = environment
    config["EBAY_API_BASE"] = configured
    service = MediaService(config, token_provider=lambda: "token")
