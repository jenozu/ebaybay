...
def test_csrf_blocks_unprotected_post(tmp_path):
    ...
    assert client.post("/settings/ebay/defaults/refresh").status_code == 400
    assert client.post("/settings/ebay/defaults").status_code == 400
    assert client.post("/settings/ebay/inventory-locations").status_code == 400
